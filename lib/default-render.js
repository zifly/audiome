...
var g_songslist=new Array();//歌单列表
...
var g_lyric=new Array();
var g_m3u=new Array();
var g_listflag //传回来的list是playlist or album 
var g_catname='全部'
...
document.getElementById('newbookimg').onclick=function(){
    arg=getTextFile('../htmls/audio-new.html');
    sobj=document.getElementById('popwin');
    sobj.innerHTML=arg;
...
ipc.on("mp3url",function(event,url,flag){
    //alert(url)
    if(url!=null){
        bgMusic.src=url;
        if(flag==null){
...
ipc.on("playlist-info",function(event,objs){
    ...
    mycon=getTextFile("../htmls/playlist-detail.html")
    ...
    if(g_songslist.length>tracks.length) g_songslist.length=tracks.length
    g_songslist=tracks;
...
ipc.on("albumlist-info",function(event,listobj){
    ...
    songs=listobj['songs'];
    if(g_songslist.length>songs.length) g_songslist.length=songs.length
    for(snum=0;snum<songs;snum++){
        songs[snum]['_listname']=listname;
        g_songslist[snum]=songs[snum];
    }
...
ipc.on('return-new-lists',function(event,args){
    ...
    mydiv=getTextFile("../htmls/playlistcover.html")
    ...
    conobj=document.getElementById('listall');
    conobj.innerHTML=alldiv;
    topobj=document.getElementById('toplistdiv');  
...
ipc.on('begin-output',function(event,name1){
    popstr="歌曲《"+name1+"》开始导出...";
    showpopwin(popstr,-1)
})
...
ipc.on('finish-output',function(event){
    ...
    if(g_copylist.length>0){
        ...
        setTimeout(function(){
            ipc.send('output-lyric',g_lyric[0])
        },2)
...
ipc.on('finish-lyric',function(event){
        g_lyric.shift();
        if(g_lyric.length>0){
            setTimeout(function(){ipc.send('output-lyric',g_lyric[0]);},1)
        } 
        else{
            ipc.send('output-m3u',g_m3u)
            popstr="开始导出m3u歌单文件...";
            showpopwin(popstr,-1)
        }   
})
...
function refreshlist(){
    ...
    switch(g_listflag){
        case 'playlist':
        ipc.send('get-play-list',g_listid)
        break;
        case 'album':
        ipc.send('get-album-list',g_listid)
        break;
    }
...
function downallsongs(){
    ...
    for(i=0;i<g_songslist.length;i++){
        if(g_songslist[i]['_localFile']=='') {
            downid=g_songslist[i]['id'];
            existid=false;
            for(m=0;m<g_downlist.length;m++){
                haveid=g_downlist[m]['id'];
            ...
            if(existid==false){
                g_downlist.push(g_songslist[i]);
                kk++;
            }
        }
    }
        alertstr="有 "+kk+" 首歌添加下载<br>";
        alertstr=alertstr+"重复 "+renum+" 首歌<br>"
...
function getsongslist(songs){
    liststr="";
    mdlist=getTextFile("../htmls/songlist.html")
for(i=0;i<songs.length;i++){
    ...
    divstr=divstr.replace('[songalbumname]',albumname)
    divstr=divstr.replace('[albumid]',albumid)
...
function audiodown(idx){
    ...
    myid=mysong['id'];
    existid=false;
    for(i=0;i<g_downlist.length;i++){
        haveid=g_downlist[i]['id'];
        if(haveid==myid){
            existid=true;break
        }
    }
...
function clearplaysongslist(){
 ...
 function backtohome(){
    ...
    ipc.send('get-top-lists',g_catname)
 }


function outputsongs(){
  alertstr="正在准备导出歌单数据..."
    showpopwin(alertstr,-1);
    setTimeout(function(){
        for(i=0;i<g_songslist.length;i++){
            g_copylist[i]=g_songslist[i];
            g_lyric[i]=g_songslist[i];
            g_m3u[i]=g_songslist[i];
        }
        ipc.send('output-songs',g_copylist[0]);
    },2)
...
function gettoplist(s){
    
    g_catname=s;
    ipc.send('get-top-lists',s)

    for(i=1;i<8;i++){
        catobj=document.getElementById('cat'+i);
        if(catobj.innerText==s){
            catobj.style.color="#990000"
        }
        else catobj.style.color="#000000"
    }
