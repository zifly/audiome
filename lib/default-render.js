...
let txtEditor = document.getElementById('maindo2'); //获得TextArea文本框的引用 
...
function downallsongs(){
    ...
    for(i=0;i<g_songslist.length;i++){
        if(g_songslist[i]['localFile']=='') g_downlist.push(g_songslist[i])
    }
...
function playthissong(){
    mytitle=g_nowsong['name'];
        ...
        if(g_nowsong['localFile']!=''){
            bgMusic.src=g_nowsong['localFile'];
            
            goto();
            mycover=getcoverfromfile(g_nowsong['localFile']);
            cobj=document.getElementById('audiocover');
            cobj.src=mycover;
